...
- `extract_panda_seq.py`: downloads chapters sequentially and saves to `novel.txt`.
- `extract_panda_para.py`: downloads chapters concurrently (parallel) and saves to `<BookTitle>.txt`. 
- `novel_scraper.py`: a more generic scraper with retries + “next page” handling for multi-page chapters.
...
- Optional: **multi-page chapter** support (clicking “下一页”) and **retry logic** in `novel_scraper.py`.
